...
export const itemStyles = tv({
	extend: focusRing,
	base: [
		"group relative flex flex-row items-center justify-between gap-2 cursor-default select-none py-1.5 px-2.5 rounded-md will-change-transform text-sm forced-color-adjust-none",
	],
	variants: {
		isSelected: {
...
export interface DropdownSectionProps<T extends Key> extends SectionProps<T> {
	title?: string;
	options?: Option<T>;
}

export function DropdownSection<T extends Key>(props: DropdownSectionProps<T>) {
	return (
